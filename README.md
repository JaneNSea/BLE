...
cover: "/images/notes/ble-connection-timeout/cover.webp"
coverPosition: "50% 50%"
authors:
  - name: "JaneNSea"
    role: "作者"
...
`tags` 用于详细页，可填写具体框架、协议和技术概念；`listingTags` 只用于列表卡片与筛选，建议保留“开发方向 + 编程语言”，例如 `前端 + TypeScript` 或 `嵌入式 + C`。`authors` 支持多人署名；`cover` 用于技术笔记详情页右侧小图，`coverPosition` 控制图片在不规则裁切中的焦点。短文章推荐使用 Markdown，并保持“问题 → 原因 → 解法 → 边界”的结构。Codex 跨会话录入技术文档时必须遵循 [`docs/CODEX_TECHNICAL_NOTE_GUIDE.md`](./docs/CODEX_TECHNICAL_NOTE_GUIDE.md)。
